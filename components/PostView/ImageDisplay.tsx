  ...
  if (imageIds.length === 1) {
    return (
      <TouchableOpacity onPress={onImagePress}>
        <Image
          source={{ uri: getImagePreview(imageIds[0], 30, 480, 360) }}
      ...
      <TouchableOpacity onPress={onImagePress}>
        <View className="h-auto w-full flex-row">
          {imageIds.map((imageId, key) => (
            <Image
              key={key}
              source={{ uri: getImagePreview(imageId, 30, 240, 426) }}
      ...
      <TouchableOpacity onPress={onImagePress}>
        <View className="relative h-auto w-full flex-row">
          {imageIds.slice(0, 2).map((imageId, key) => (
            <Image
              key={key}
              source={{ uri: getImagePreview(imageId, 30, 360, 480) }}
              className="w-1/2 h-64 bg-panel"
              resizeMode="cover"
            />
          ))}
      ...
      <TouchableOpacity onPress={onImagePress}>
        <View className="relative h-auto w-full flex-row flex-wrap">
          {imageIds.map((imageId, key) => (
            <Image
              key={key}
              source={{ uri: getImagePreview(imageId, 30, 426, 240) }}
